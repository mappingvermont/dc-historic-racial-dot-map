...
def main():
    ...
    parser.add_argument('--tract-geojson', '-t', help='path to tract geojson', required=True)
    ...
    gdf = gpd.read_file(args.tract_geojson)
    df = pd.read_csv(args.population_csv)

    # filter geojson to only include DC
    try:
    ...
    # project input geojson to WGS84
